const Fft = function (radix) {
  ...
  const self = {
    forward: function (b, window) {
      let i, j, n, k, k2, h, d, c, s, ik, dx, dy;
      n = fft.buffer.length;
      const winFunction = {
        name: window,
        N: n,
    ...
    inverse: function (re, im) {
      ...
      for (i = n; i--; ) {
        // ùpdifié pour retourner la valeur absolue complexe et pas que la partie imaginaire
        fft.buffer[i] = sqrt((fft.reI[i] / n) ** 2 + (fft.imI[i] / n) ** 2);
      }
      return fft.buffer;
    },
    ...
    windows: function () {
      const winFuncs = [];
      for (let k in windowCalculation) {
        winFuncs.push(k);
      }
      return winFuncs;
    },
